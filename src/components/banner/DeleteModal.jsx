...
const DeleteModal = ({data, banners_get}) => {
    //id of category
    const {id} = data;

    // //on click yes,delete
    const deleteRequest = async() => {
        let response = await ApiClass.deleteRequest(`banner/delete?id=${id}`,true);
        ...
        if(response?.data?.status_code == 1){
            ...
            document.getElementById('closeDeleteBanner').click();
    ...
    return(
        <>
            <div className="modal fade common-modal" id="deleteBannerModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
