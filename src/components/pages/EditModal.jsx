...
const EditModal = ({ senddata,getPages }) => {
    const { slug, content, type, extra, sub_type, id } = senddata;
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoading(true)
            let response = await ApiClass.putNodeRequest(`admin/pages/update?id=${id}`, true, values);
            setLoading(false)
            if (response?.data?.status_code == 1) {
                SwalClass.success(response?.data?.message)
                document.getElementById('pagedEditModalbutton').click()
                getPages();
            }
    ...
    useEffect(() => {
        if (senddata != undefined) {
            ...
            setFieldValue("sub_type", sub_type)
    ...
    return (
        <>
            <div className="modal fade common-modal" id="pagedEditModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    ...
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title text-uppercase d-flex align-items-center" id="blogEditModalLabel">
                                ...
                                Edit {slug} </h5>
                            <button type="button" className="btn p-0 border-0" data-bs-dismiss="modal" aria-label="Close" id="pagedEditModalbutton">
