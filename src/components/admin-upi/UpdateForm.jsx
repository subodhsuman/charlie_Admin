...
const UpdateForm = ({data, getUpi}) => {
    
    const {id, upi_id} = data;
    ...
    const formik = useFormik({
        ...
        onSubmit: async(values)=>{
            setLoading(true)
            let response = await ApiClass.updateRequest(`adminupis/update/${id}`,true,values);
            setLoading(false)
    ...
    useEffect(()=>{
        if(upi_id != undefined){
            setFieldValue("upi_id",upi_id)
        }
    },[data]);
    return(
        <div className="modal fade common-modal" id="updateUpiModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                ...
                                <div className="row">
                                    <div className="col-md-12">
                                        <div className="mb-3">
                                            ...
                                            <input  className="form-control" type="text" placeholder="Enter Upi Id" name="upi_id" value={values.upi_id || ''} onChange={handleChange} />
                                            {errors.upi_id && touched.upi_id && (<span className="text-danger form_err">{errors.upi_id}</span>)}
                                        </div>
                                    </div>
