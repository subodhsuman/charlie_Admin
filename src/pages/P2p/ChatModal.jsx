...
const ChatModal = ({ user, cryptosedit }) => {
    return (
        <>
            <div className="modal fade common-modal" id="chatEditModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div className="modal-dialog modal-lg modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title text-uppercase d-flex align-items-center" id="ChatEditModal">
                        ...
                        <div className="modal-body admin-form">
                            <div className="chat-scroller px-3" style={{ maxHeight: '400px', overflowY: 'scroll' }}>
                                {cryptosedit?.length != 0 ?
                                    <>
                                        {cryptosedit?.map((v, index) => {
                                            return (
                                                <React.Fragment key={index}>
                                                    <div className={`message-box d-flex ${v.user.name == user ? 'justify-content-start box-left' : 'justify-content-end box-right'}`} 
                                                   
                                                    >
                                                        <div className="mb-0">
                                                            {(v.message.length == 0 ? '' : <p className="mb-0 fw-bold" style={{ fontSize: '16px' }}>{v?.user?.name}</p>)}
                                                            {v?.image ?
                                                                <a href={`${ApiClass.nodeUrl}chatImages/${v.image}`} target="blank">
                                                                    <img src={`${ApiClass.nodeUrl}chatImages/${v.image}`} alt=""  className="mb-2" width="200" />
                                                                </a>
                                                                :
                                                                ""}
                                                            {v.message.length == 0 ? '' :
                                                                <>
                                                                    <p className="mb-0 p-2 rounded" style={{ backgroundColor: (v.user.name == user ? 'var(--red)' : 'var(--blue)'), wordBreak:'break-all'}}>{v.message}</p>
                                                                    <p className="mb-3" style={{ fontSize: '11px' }}>{new Date(v?.created_at).toLocaleString()}</p>
                                                                </>
                                                            }
                                                        </div>
                                                    </div>
                                                     {/* <p className={`mb-3 ${v.user.name == user ? 'text-start' : 'text-end'}`} style={{ fontSize: '11px' }}>{new Date(v?.created_at).toLocaleString()}</p> */}
                                                </React.Fragment>
                                            )
                                        }).reverse()}
                                    </>
