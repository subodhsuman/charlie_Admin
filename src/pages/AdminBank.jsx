...
import Table from "../components/admin-bank/Table";
import UpdateForm from '../components/admin-bank/UpdateForm';
import Form from "../components/admin-bank/Form";
...
const AdminBank = () => {
    ...
    const getBank = async() => {
        let response = await ApiClass.getRequest('adminbanks/get',true);
 ...
 //update status
 const updateBank = async(status,id) => {
    status = status ? 1 : 0;
    let response = await ApiClass.updateRequest(`adminbanks/account_status/${id}/${status}`,true);
    if(!response?.data.hasOwnProperty("status_code")){
        SwalClass.failed("Unable to Update record at this time");
        return;
    }
    if(response?.data?.status_code == 0){
        return
                ...
                <section className="dashboard-sec">
                    <div className="container-fluid">
                        <div className="dashboard-heading mb-3 d-flex align-items-center justify-content-between">
                            ...
                            <button className="btn p-0 border-0" data-bs-toggle="modal" data-bs-target="#adminBankModal">
